...
/// committed binary files (used for known-property assertions like KnownAuthor),
...
internal static class SampleDocuments
{
    ...
    private static byte[] BuildAuthoredPdf(string title, string author)
    {
        ...
        WriteObj($"4 0 obj\n<< /Title ({EscapePdfString(title)}) /Author ({EscapePdfString(author)}) /Creator (GroupDocs.Conversion.Mcp integration tests) >>\nendobj\n");

...
The `GroupDocs.Conversion.Mcp` package is an **MCP server** (the csproj declares
...
[nuget.org/packages/GroupDocs.Conversion.Mcp](https://www.nuget.org/packages/GroupDocs.Conversion.Mcp)
...
```bash
curl -L -o pkg.nupkg \
  https://www.nuget.org/api/v2/package/GroupDocs.Conversion.Mcp/26.5.0
...
```json
{
  ...
  "name": "io.github.groupdocs-conversion/groupdocs-conversion-mcp",
  "version": "26.5.0",
  "packages": [ { "registryType": "nuget", "identifier": "GroupDocs.Conversion.Mcp", ... } ],
  "repository": { "url": "https://github.com/groupdocs-conversion/GroupDocs.Conversion.Mcp" }
}
```
...
3. Search for `groupdocs-conversion` or `conversion`.
4. The package should appear with the description from `server.json`:
   *"MCP server for GroupDocs.Conversion — convert documents between PDF, Word, Excel, PowerPoint, HTML, and 70+ formats, list supported conversions, and inspect document info via AI agents."*
...
- `dnx` successfully resolves `GroupDocs.Conversion.Mcp@26.5.0` from nuget.org.
...
curl -s https://registry.modelcontextprotocol.io/v0/servers \
  | jq '.[] | select(.name | contains("groupdocs-conversion"))'
```
...
- `ToolDiscoveryTests.ServerInfo_AdvertisesGroupDocsConversionMcp` — the server
  reports `serverInfo.name == "GroupDocs.Conversion.Mcp"` and a non-empty version
  that matches the package under test.
- `ToolDiscoveryTests.ListTools_ExposesConvertSupportedFormatsAndDocumentInfo` — all
  three MCP tools are advertised.

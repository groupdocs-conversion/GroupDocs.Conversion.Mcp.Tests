...
- `ghcr.io/groupdocs-conversion/conversion-net-mcp` — GitHub Container Registry (primary)
- `docker.io/groupdocs/conversion-net-mcp` — Docker Hub (mirror)
...
```bash
docker pull ghcr.io/groupdocs-conversion/conversion-net-mcp:26.5.0   # pinned to exact version
docker pull ghcr.io/groupdocs-conversion/conversion-net-mcp:latest   # floats to most recent push
```
...
docker pull ghcr.io/groupdocs-conversion/conversion-net-mcp:latest
...
```bash
docker inspect ghcr.io/groupdocs-conversion/conversion-net-mcp:latest \
...
(
  ...
  sleep 2
    ...
    -v $(pwd)/documents:/data \
    ghcr.io/groupdocs-conversion/conversion-net-mcp:26.5.0 2>/dev/null
```

Expected: two JSON-RPC responses on stdout. The second includes `convert`
and `get_supported_formats` with their descriptions and input schemas.
...
services:
  groupdocs-conversion-mcp:
    image: ghcr.io/groupdocs-conversion/conversion-net-mcp:26.5.0
    stdin_open: true
    tty: false
    environment:
...
Without the license, `get_supported_formats` returns the evaluation-mode error
...
docker inspect ghcr.io/groupdocs-conversion/conversion-net-mcp:26.5.0 \
...
| `manifest unknown` / can't pull image | Version tag doesn't exist on that registry | Check [ghcr.io/groupdocs-conversion/conversion-net-mcp](https://github.com/groupdocs-conversion/GroupDocs.Conversion.Mcp/pkgs/container/conversion-net-mcp) for available tags. |
| `:latest` ran but didn't pick up a new release | Docker reuses cached image — `docker run :latest` does not auto-pull | `docker pull ghcr.io/groupdocs-conversion/conversion-net-mcp:latest` before running, or add `--pull always` to your `docker run`. |

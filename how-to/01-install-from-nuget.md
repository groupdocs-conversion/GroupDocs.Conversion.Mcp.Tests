...
Two install routes from [`nuget.org/packages/GroupDocs.Conversion.Mcp`](https://www.nuget.org/packages/GroupDocs.Conversion.Mcp):
...
```bash
dnx GroupDocs.Conversion.Mcp@26.5.0 --yes
```
...
```bash
dnx GroupDocs.Conversion.Mcp --yes                # latest stable, refreshed every run
dnx GroupDocs.Conversion.Mcp --prerelease --yes   # latest including pre-releases
```
...
The cache lives at `%USERPROFILE%\.nuget\packages\groupdocs.conversion.mcp\<version>\`
on Windows and `~/.nuget/packages/groupdocs.conversion.mcp/<version>/` on
...
info: ModelContextProtocol.Server.StdioServerTransport[...]
      Server (stream) (GroupDocs.Conversion.Mcp) transport reading messages.
...
(
  ...
  sleep 2
) | GROUPDOCS_MCP_STORAGE_PATH=./docs dnx GroupDocs.Conversion.Mcp@26.5.0 --yes
```

You should see two JSON-RPC responses containing the three advertised tools:
`Convert`, `GetSupportedFormats`, and `GetDocumentInfo`.
...
```bash
dotnet tool update -g GroupDocs.Conversion.Mcp
```

To uninstall:

```bash
dotnet tool uninstall -g GroupDocs.Conversion.Mcp
```

The tool runs from `~/.dotnet/tools/groupdocs-conversion-mcp` (Linux/macOS) or
`%USERPROFILE%\.dotnet\tools\groupdocs-conversion-mcp.exe` (Windows). Make sure
...
|---|---|---|
| `GROUPDOCS_MCP_STORAGE_PATH` | Folder the server reads input from and writes converted output to | current working directory |
| `GROUPDOCS_MCP_OUTPUT_PATH` | Optional — route output files to a different folder | same as storage |
| `GROUPDOCS_LICENSE_PATH` | Path to `GroupDocs.Total.lic` — drops evaluation-mode watermarks from converted output | *(evaluation mode — output watermarked)* |
...
All three tools (`Convert`, `GetSupportedFormats`, `GetDocumentInfo`) work in
evaluation mode without a license. `Convert` produces output files but with an
evaluation-mode watermark; the tool's response includes a
`[Evaluation mode]` prefix to flag this. `GetSupportedFormats` and
`GetDocumentInfo` are read-only and unaffected.

Get a license from [purchase.groupdocs.com](https://purchase.groupdocs.com/).
Point `GROUPDOCS_LICENSE_PATH` at the `.lic` file and `Convert` output loses
the watermark and the evaluation-mode prefix.
...
```text
initialize response → serverInfo: { name: "GroupDocs.Conversion.Mcp", version: "26.5.0" }
```
...
| `No license configured. Running in evaluation mode.` | No `GROUPDOCS_LICENSE_PATH` | Expected. `Convert` output will be watermarked; the read-only tools are unaffected. Set `GROUPDOCS_LICENSE_PATH` to drop the watermark. |

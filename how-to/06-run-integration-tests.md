...
This repo's test suite validates the **published** `GroupDocs.Conversion.Mcp`
...
```bash
# All 12 tests against the default pinned version (26.5.0)
...
Three tests in `GetSupportedFormatsTests` only assert success when a GroupDocs
...
# Only convert tests
dotnet test -c Release --filter "FullyQualifiedName~Convert"
...
Passed  ToolDiscoveryTests.ListTools_ExposesConvertSupportedFormatsAndDocumentInfo
Passed  ToolDiscoveryTests.AllTools_HaveNonEmptyDescriptionAndInputSchema
Passed  ConvertTests.Convert_AuthoredPdf_ToHtml_ProducesOutputFile
Passed  ConvertTests.Convert_RealSample_ProducesExpectedOutputFile (× 4 theory rows)
Passed  ConvertTests.Convert_UnsupportedTargetFormat_ReturnsClearMessage
Passed  GetSupportedFormatsTests.GetSupportedFormats_AuthoredPdf_ReturnsTargetList
Passed  GetSupportedFormatsTests.GetSupportedFormats_PlainJpeg_ReturnsImageTargets
Passed  GetSupportedFormatsTests.GetSupportedFormats_RealSample_AdvertisesNonEmptyTargets (× 4 theory rows)
Passed  GetDocumentInfoTests.GetDocumentInfo_AuthoredPdf_ReturnsInfoTypeAndProperties
Passed  GetDocumentInfoTests.GetDocumentInfo_AuthoredPdf_SurfacesKnownAuthorOrTitle
Passed  GetDocumentInfoTests.GetDocumentInfo_RealSample_ReturnsInfoType (× 4 theory rows)
...
Passed  ErrorHandlingTests.PasswordParameter_IsAcceptedByConvert
...
Synthetic PDF/JPEG fixtures ([SampleDocuments.cs](../src/GroupDocs.Conversion.Mcp.Tests/Fixtures/SampleDocuments.cs))
...
- name: Dispatch integration tests
  ...
  run: |
    gh api \
      repos/groupdocs-conversion/GroupDocs.Conversion.Mcp.Tests/dispatches \
...
base64 -w0 GroupDocs.Total.lic | gh secret set GROUPDOCS_LICENSE \
  --repo groupdocs-conversion/GroupDocs.Conversion.Mcp.Tests
```
...
test fails with a cryptic `"An error occurred invoking 'get_supported_formats'"`,
...
(
  ...
  echo '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"get_supported_formats","arguments":{"file":{"filePath":"some.pdf"}}}}'
  sleep 5
) | GROUPDOCS_MCP_STORAGE_PATH=/tmp/gd dnx GroupDocs.Conversion.Mcp@26.5.0 --yes \
...
| All GetSupportedFormats tests fail | Running unlicensed. Expected — `GetSupportedFormats_InEvaluationMode_ReturnsErrorResponse` should pass, the `*_Licensed` tests no-op. If the eval-mode test also fails, check the package actually installed. |

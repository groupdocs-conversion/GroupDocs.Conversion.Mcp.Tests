...
#
# Launches `ghcr.io/groupdocs-conversion/conversion-net-mcp` as an interactive MCP
...
#
# Options:
#   --image-tag TAG         Image tag (default: latest). Pin to e.g. "26.5.2"
...
#                           advertise both `convert` and `get_supported_formats`.
#   --license PATH          Mount a GroupDocs license file (enables
#                           get_supported_formats; without it the server runs in
#                           evaluation mode)
...
#   ./04_run-server-with-samples.sh --image-tag 26.5.2   # pinned
...
IMAGE_REPO="ghcr.io/groupdocs-conversion/conversion-net-mcp"
...
# nothing to /data when only running convert / non-licensed get_supported_formats.
...
if [ -n "$LICENSE_PATH" ]; then
    ...
    log_info "License mounted — get_supported_formats will work in licensed mode"
else
    log_info "No license — get_supported_formats will return evaluation-mode error (convert works fine)"
fi
...
if [ "$RUN_MODE" = "smoke" ]; then
    ...
    if echo "$response" | grep -q '"convert"' && echo "$response" | grep -q '"get_supported_formats"'; then
